...
### Checkout propio construido (15-sep-2026, tarde — rama `feature/checkout-propio`, SIN desplegar a producción)

- Contrato y desviaciones reales: **`CHECKOUT-PROPIO.md`** (§9 = lo que hay en el código). Producción sigue con el motor `iframe`; el motor `native` (entradas + mesas VIP + listas sobre la Channel Manager API, pago por redirección a `payment_url`) se activa con `CHECKOUT_ENGINE=native` o `?engine=native` fuera de producción.
- Piezas: `src/lib/checkout/{engine,validation}.ts`, `src/lib/fourvenues/{client,types,pricing}.ts`, rutas `src/app/api/checkout/{pricing,discount,tickets,booking,list}` + `src/app/api/fourvenues/webhook`, UI `src/app/taquilla/native/*`, claves `checkout.*` en los 5 diccionarios, mock `e2e/mocks/fourvenues-mock.mjs` y `e2e/checkout-native.spec.ts` (12 escenarios), `scripts/fourvenues-register-webhook.mjs` (no ejecutado).
- Verificado: `tsc` 0 errores, eslint limpio en el cambio, `next build` en verde, suite Playwright completa en verde (59 pasados, 1 omitido por diseño), recorrido manual a 375 px con capturas en `../checkout-propio-capturas/`.
- Conmutación y vuelta atrás: CHECKOUT-PROPIO.md §8 (env `CHECKOUT_ENGINE`, registrar webhook y guardar `FOURVENUES_WEBHOOK_SECRET`, compra real de prueba). Pendiente de Jose: decidir cuándo. **OJO:** cambiar una variable de entorno en Vercel NO redespliega solo: tras poner `CHECKOUT_ENGINE=iframe` (vuelta atrás) hay que hacer Deployments → Redeploy del último deploy de producción (o «Promote to Production» del deploy anterior a la fusión, aún más rápido).
- Panel de revisión (15-sep, noche; agentes código/SecOps/pagos/web/legal/a11y) aplicado: total real de Fourvenues contrastado antes de redirigir (entradas y mesas), sin cifras de stock en el HTML, casilla de imágenes sustituida por aviso informativo, info básica RGPD y avisos de no devolución/depósito en el resumen, condiciones de venta y privacidad reescritas para el proceso real (5 idiomas), Purchase con importe verificado, timeouts, rate limit de listas, accesibilidad (tamaños ≥14 px, foco, pestañas, contraste, banner de cookies sobre la barra). Detalle y dudas abiertas: CHECKOUT-PROPIO.md §9.5.
